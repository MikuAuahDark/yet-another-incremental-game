...
local text = {
    ...
    PAUSE = loc("Pause", nil, {
        context = "A button to pause the game"}),
    PAUSE_DESCRIPTION = loc("Pauses the game. You can also press ESC key to pause the game.", nil, {
        context = "A description on what \"Pause\" button is for."}),
    HIDE_HUD = loc("Hide UI", nil, {
        context = "A button to hide the HUD"}),
    HIDE_HUD_DESCRIPTION = loc("Hides the HUD and the UI. Click anywhere to show it back.", nil, {
        context = "A description on what \"Hide HUD\" button is for."}),
    RESET_CAMERA = loc("Reset Camera", nil, {
    ...
    REQUIRES = loc("Requires:", nil, {
        context = "Denotes the data requirement of a machine."}),
    PROVIDES = loc("Provides:", nil, {
        context = "Denotes the data provided by a machine."}),
    DATA_REQUIREMENT_WORLD = interp("%{amount}/%{required} %{data}", {
        context = "Denotes the data requirement of a machine."}),
    DATA_REQUIREMENT_HUD = interp("%{required} %{data}", {
        context = "Denotes the data requirement of a machine."}),
    DATA_PROVIDE = interp("%{value} %{data}/sec", {
        context = "Denotes the data provided by a machine."}),
    ...
    LOAD_TOOLTIP = interp("{COLORS_UI_TEXT_POWER_RELATED}%{load}{bolt}{/COLORS_UI_TEXT_POWER_RELATED} Power", {
